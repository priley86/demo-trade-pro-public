...
export const GetOrderHistorySchema = z.object({
  ...
  status: z.enum(['pending', 'filled', 'cancelled', 'rejected', 'all'])
    .default('all')
...
export function createAISDKTool(apiClient: DemoTradeProAPIClient) {
  return tool({
    ...
    execute: async ({ userId, limit = 20, status = 'all', symbol }) => getOrderHistoryHandler({ userId, limit, status, symbol }, apiClient)
  })
}

// MCP tool factory
export function createMCPTool(handler: (params: GetOrderHistoryParams) => Promise<Order[]>) {
  return {
