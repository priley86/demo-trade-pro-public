...
* ❌ **Root keys** (M2M credentials) = unlimited power, single point of failure
* ❌ **JWT forwarding** = vulnerable to replay attacks if leaked  
* ✅ **Token Vault** = enterprise-grade security with zero custom code
...
✨ **Magic:** This automatically enables Token Vault and other GenAI features!

## 2 – Configure OpenID Connection
...
## 4 – Update Agent Tools to Use Vault
...
export async function POST(request: Request) {
  ...
  const result = await streamText({
    ...
    tools: {
      ...
      placeOrder: placeOrderTool  // ✅ Secure with 5-minute tokens!
    },
  })
...
🎆 **Success!** Your agent now uses enterprise-grade Token Vault security!

---

## ✨ What Just Happened?

- ✅ **No secrets in agent code** – all handled by Auth0
- ✅ **5-minute token expiry** – minimal attack window
- ✅ **Scoped permissions** – only `trade:write` when needed
...
> **Tip:** Logs show each access token expires after 5 minutes—replay attacks are basically useless.
