...
> **Goal:** Replace individual agent tools with one MCP server connection.
> **Estimated time:** 12 minutes (5 min intro + 7 min anti-pattern)
...
const tools = await mcpClient.listTools()  // ✨ Magic!
```
...
## 2 – Update Chat Route to Use MCP
...
export async function POST(request: Request) {
  ...
  const result = await streamText({
    ...
    messages,
    tools,  // 🎆 All tools from MCP server!
  })
...
3. **But now powered by MCP!** 🎆

✨ **The Magic:** Your agent got SIMPLER, not more complex! All tools now come from the MCP server.

---

## ⚠️ MCP JWT Forwarding Anti-Pattern

**But wait... how does the MCP server authenticate API calls?**

### 😱 The Wrong Way: JWT Forwarding

```typescript
// apps/mcp-server/src/tools/placeOrder.ts - DON'T DO THIS!
export async function placeOrder(args: any, context: { userJWT: string }) {
  // 😱 Storing user JWT in MCP server!
...
- ❌ **Replay attacks** if MCP server is compromised
- ❌ **Long-lived tokens** stored in memory
- ❌ **No audit trail** of who initiated the request
- ❌ **Token theft** from logs/memory dumps

### ✅ The Right Way: MCP + Token Vault
...
export async function placeOrder(args: any, context: { userId: string }) {
  // ✅ Get fresh token from Auth0 Token Vault
...
- ✅ **No JWT storage** in MCP server
- ✅ **Fresh tokens** for each request
- ✅ **Proper user context** for auditing
- ✅ **Enterprise security** + tool sharing

---

## 🎆 What You Just Built

- ✅ **Agent simplified** from many tools → one MCP client
- ✅ **Tools centralized** in reusable MCP server
- ✅ **Security maintained** with Token Vault
- ✅ **Scalable architecture** - other agents can use same MCP server!

**Bonus:** ChatGPT, Claude, or Cursor can now use your secure DemoTradePro MCP server! 🤯

🎉 **Workshop complete!** You've built enterprise-grade AI agent security.
